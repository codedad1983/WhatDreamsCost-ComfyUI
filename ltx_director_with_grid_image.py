...
class LTXDirectorWithGridImage(io.ComfyNode):
    """Automatic storyboard-to-LTX director node for batch images and LLM shot text."""

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="LTXDirectorWithGridImage",
            display_name="LTX Director With Grid Image",
    ...
    def execute(cls, model, clip, storyboard_images, llm_response, global_prompt="",
        ...
        if storyboard_images is None or storyboard_images.shape[0] < 1:
            raise ValueError("LTX Director requires at least one storyboard image.")
        ...
        if len(prompts) < count:
            log.warning(
                "[LTX Director] Parsed %d prompts for %d images. Filling missing prompts with generic motion prompts.",
                len(prompts), count,
            )
        ...
        if optional_latent is None:
            ...
            log.info(
                "[LTX Director] Auto-generated LTXV latent: %dx%d, %d pixel frames (%d latent frames)",
