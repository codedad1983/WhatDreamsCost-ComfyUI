...
class LTXDirectorWithMoreOutput(io.ComfyNode):
    """WYSIWYG timeline variant — segments and lengths come from a visual editor in the node UI."""

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="LTXDirectorWithMoreOutput",
            display_name="LTX Director With More Output",
            ...
            outputs=[
                ...
                # New outputs (4 additional)
                io.String.Output(display_name="summary_prompt", tooltip="All prompts concatenated (global + segments)"),
                io.Image.Output(display_name="first_guide_image_raw", tooltip="First guide image as originally loaded (no resize/compression)"),
                io.Int.Output(display_name="total_frames", tooltip="Total number of pixel frames"),
                io.Float.Output(display_name="total_duration_sec", tooltip="Total duration in seconds"),
            ],
        )
    ...
    def execute(cls, model, clip, global_prompt, duration_frames, duration_seconds,
        ...
        first_raw_image = None   # store the very first original image (before any transform)
        ...
        try:
            ...
            for idx, seg in enumerate(img_segs):
                # Load the original image (no transforms yet)
                original_tensor = _load_image_tensor(seg)
                # Store the first original image (as is) for output
                if idx == 0 and first_raw_image is None:
                    first_raw_image = original_tensor.clone() if original_tensor.numel() > 0 else None

                tensor = original_tensor  # start from original
            ...
            if not guide_data["images"]:
                ...
                # Also create a dummy original image (black) for consistency
                if first_raw_image is None:
                    first_raw_image = torch.zeros((1, h, w, 3), dtype=torch.float32)

        except Exception as e:
            log.warning("[PromptRelay] Could not build guide_data: %s", e)
            # Fallback: ensure first_raw_image exists
            if first_raw_image is None:
                h = derived_h if derived_h > 0 else 512
                w = derived_w if derived_w > 0 else 768
                first_raw_image = torch.zeros((1, h, w, 3), dtype=torch.float32)
        ...
        # Total frames (pixel space) based on actual latent
        latent_samples = latent["samples"]
        _, _, temporal_latent, _, _ = latent_samples.shape
        total_frames_val = (temporal_latent - 1) * 8 + 1   # LTXV compression factor = 8
        total_duration_sec_val = total_frames_val / frame_rate

        # --- Summary prompt: concatenate global and local prompts without any extra markers ---
        all_prompts = []
        if global_prompt.strip():
            all_prompts.append(global_prompt.strip())
        if local_prompts:
            segments = [p.strip() for p in local_prompts.split("|") if p.strip()]
            all_prompts.extend(segments)
        summary_prompt_text = "\n".join(all_prompts) if all_prompts else ""

        # Ensure first_raw_image is a valid tensor (should be set already)
        if first_raw_image is None:
            # Fallback: create a dummy black image of reasonable size
            first_raw_image = torch.zeros((1, 512, 512, 3), dtype=torch.float32)

        # --- Return with new outputs (7 original + 4 new = 11) ---
        return io.NodeOutput(
            patched, conditioning, latent, audio_latent, guide_data, float(frame_rate), audio_out,
            summary_prompt_text, first_raw_image, total_frames_val, total_duration_sec_val
        )
