...
from .ltx_director_with_grid_image import (
    MAX_AUTO_SEGMENTS,
    _empty_audio_latent,
    _normalize_lengths,
    _parse_prompts,
...
class LTXDirectorWithGridImageV2(io.ComfyNode):
    """Original LTX Director timeline with automatic grid image and LLM prompt fill."""

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="LTXDirectorWithGridImageV2",
            display_name="LTX Director With Grid Image V2",
        ...
        if audio_vae is not None:
            if use_custom_audio:
                try:
                    ...
                    log.error("[LTX Director] Failed to encode custom audio: %s", exc)
                    raise exc
            else:
